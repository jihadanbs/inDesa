...
<?php 
$koneksi = mysqli_connect('localhost', 'root', '', 'inDesa');
...
function register($dataRegister) {
    ...
    $namaDesa = mysqli_real_escape_string($koneksi, htmlspecialchars( $dataRegister['nama-desa'] ));
    $paket = mysqli_real_escape_string($koneksi, htmlspecialchars( $dataRegister['paket'] ));
    $order_id= rand();
    $biaya = 100000;
    ...
    $no_rek = acak(12);
    $sukses = mysqli_query($koneksi, "INSERT INTO users (email, username, password, no_rek, nama_desa, paket, order_id, biaya, transaction_status, transaction_id)
     VALUES ('$email', '$username', '$password', '$no_rek', '$namaDesa', '$paket', '$order_id', '$biaya', '$transaction_status', '$transaction_id' )");
